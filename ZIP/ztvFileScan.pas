...
Type
   ...
   TUpdateFilesList = Function(Sender: TObject; Const aDir: String;
   	aFile: TWin32FindData; pHeaderObj: pCompHeaderObj): Boolean Of Object;
...
Type
   ...
   TFilesList = Class(TBaseFilesList)
   Private
   ...
   Public
      ...
      Function DoUpdateFilesList(Sender: TObject; Const aDir: String; aFile:
      	TWin32FindData; pHeaderObj: pCompHeaderObj): Boolean; Virtual;
   ...
   Published
      ...
      Property Attributes: TztvFileAttributes Read fAttributes Write fAttributes Stored True;
      Property AttributesEx: TztvFileAttributes Read fAttributesEx Write fAttributesEx Stored True;
      ...
      // Activates only if UpdateList = true, and Assigned(UpdateFilesList)
...
Begin
   ...
   Cancel := False;
   pHeaderObj := Nil;
   UpdateFilesList := DoUpdateFilesList; // virtual procedure
...
Procedure TztvFileScan.SetAttribute(Attr: TztvFileAttribute; value: Boolean);
Begin
   If value Then
...
Begin
   ...
   Try
      ...
      Begin
         ...
         Begin
            ...
            Try
               ...
               ScanMaskCollection(fRootDir, AttributesToInt(fAttributes), MaskCollection);	// v4.7 revised
            Finally
               MaskCollection.Free();
            End;
         End
                  ...
                  Try
                     Try
                        ...
                        If Length(WorkDir) < 1 Then
                           WorkDir := AppendDirTail(GetCurrentDir());
                           ...
                           Begin
                              ...
                              Begin
                                 ...
                                 Else
                                    ...
                                    Begin
                                       FileSpecsMatch := True;
                                       TempFileSpec[k] := ExtractFilename(fFileSpec[i]);
                                       fFileSpec.Delete(i);
                                       Dec(i);
                                       break;
                                    End;
                              ...
                              Begin
                                 TempFileSpec.Add(ExtractFilename(fFileSpec[i]));
                                 fFileSpec.Delete(i);
                                 Dec(i);
                              End;
                           End;
      ...
      End Else Begin
         MaskCollection := TCollection.Create(TMultiMask);
         Try
            ScanMaskCollection('', AttributesToInt(fAttributes), MaskCollection);	// v4.7 revised
         Finally
            MaskCollection.Free();
         End;
         Exit;
...
Var
   ...
   Procedure ProcessFile(Const Dir: String);
   Var
   	m: Boolean;
   Begin
   	With FindData Do
         ...
         Begin
            ...
            Begin
               ...
               Begin
                  ...
                  If fUpdateList And Assigned(UpdateFilesList) Then
                  Begin
                     If UpdateFilesList(Self, AppendDirTail(Dir), FindData, pHeaderObj) Then
                     Begin
                        Inc(fFilesCount);

                        fFilesSize  :=
                           ((Int64(nFileSizeHigh) Shl 32) Or nFileSizeLow) +
                           fFilesSize;
...
Begin {ScanDir(...)}
   ...
   Begin
      ...
      Begin
   		Handle := Windows.FindFirstFile(PChar(FileSpec[i]), FindData);
   		LastOk := Handle <> INVALID_HANDLE_VALUE;

   		If LastOk Then
         Begin
         	//fRootDir := ExtractFilePath(FileSpec[i]);
            If Assigned(fOnRootDirChange) Then
               fOnRootDirChange(Self, ExtractFilePath(FileSpec[i]));

            Try
               While LastOk And (Not pCancel^) Do
               Begin
                  If (FindData.cFilename[0] <> '.') {And (String(FindData.cFilename) <> '..')} Then
                     ProcessFile(ExtractFilePath(FileSpec[i]));
   ...
   End Else Begin
      ...
      If LastOk Then
         Try
            While LastOk And (Not pCancel^) Do
            Begin
            If (String(FindData.cFilename)[1] <> '.') {And (String(FindData.cFilename) <> '..')} Then
               Begin
                  ProcessFile(AppendDirTail(Dir));
                  ...
                  Begin
                     ...
                     Begin
                        If fIncludeHiddenDirs Then
                           ScanDir(AppendDirTail(SlashSep(Dir, FindData.cFilename)), aAttr, aMasks);
                     End
                     Else
                        ScanDir(AppendDirTail(SlashSep(Dir, FindData.cFilename)), aAttr, aMasks);
                  End;
...
Function TztvFileScan.DoUpdateFilesList(Sender: TObject; Const aDir: String;
	aFile: TWin32FindData; pHeaderObj: pCompHeaderObj): Boolean;
Var
   f: TFileObject;
   FileTime: TFileTime;
   wDate, wTime: word;
...
Begin
   ...
   Case FSortOptions Of
      ...
      soByPath:
         If fSortCaseSen Then
            Result := AnsiCompareStr(
               TFileObject(Item1).FullPath,
               TFileObject(Item2).FullPath)
         Else
            Result := AnsiCompareText(
               TFileObject(Item1).FullPath,
               TFileObject(Item2).FullPath);
...
Procedure TFilesList.SetSortOptions(Const value: TztvSortOptions);
Begin
...
Constructor TFileObject.Create(aName, aPath: String; aSizeLow, aSizeHigh, aAttr, aTime: Integer);
Begin
   Inherited Create();
   FileName := aName;
   FullPath := aPath;
