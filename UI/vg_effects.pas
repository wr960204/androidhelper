...
uses
  ...
  {$ENDIF}
  Classes, SysUtils, vg_utils, vg_scene;
...
begin
  ...
  Off := Point(round(FDistance * Cs * Data), round(FDistance * Sn * Data));
  Visual.ApplyMask(Mask, -Off.X, -Off.Y);
  { free mask }
...
begin
  ...
  for j := 0 to c do
    if j > (c - a) then
      ...
      FillLongword(@Bits[(Visual.Height - 1 - j) * Visual.Width], Visual.Width, $0); 
end;
...
initialization
  RegisterVGObjects('Effects', [TvgShadowEffect, TvgBlurEffect, TvgGlowEffect, TvgInnerGlowEffect, TvgBevelEffect, TvgReflectionEffect]);
end.
